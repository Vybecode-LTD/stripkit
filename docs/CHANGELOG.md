# CHANGELOG â€” StripKit

> Version 0.6.0 Â· last-updated 2026-06-03 Â· last-audit 2026-06-03
>
...
- New **glassmorphism UI** (Obsidian, chosen from two rendered mockups): acrylic
  ...
  restyled. **Styling-only** â€” renderer, view-models, and tests untouched (41/41 green).
...
- **Alignment tools (pivot-only).** The renderer centres art on its detected *content*
  ...
  **Enable crosshair** toggle marks the spin centre with live drag â€” the art itself never
  moves â€” and the preview always renders at **1024 virtual steps** so alignment isn't
  ...
  **Reset**, and auto-centring of knobs on load. New `ContentAnalysis` auto-detects the
  ...
  `FilmstripEngine.cs`.
- **Preview transport.** Step back / Playâ€“Stop / step forward / **Reset** (returns to the
...
- **App icon + favicon.** A multi-resolution `.ico` (16â€“256 px) embedded via
...
### Fixed
- Off-centre knob PNGs no longer "wobble"/orbit during the sweep â€” the rotation pivot is
...
- **Velopack in-app auto-update** (`UpdateService`, `VelopackApp.Run()`, the package
  reference) â€” replaced by the Inno installer + website-download model. An Inno-installed
...
## [0.5.0] â€” 2026-06-03 â€” Meter mode

### Added
- **Phase 6 â€” Meter mode** (design signed off first). New `ComponentType.Meter`,
  `MeterFillDirection` enum, and meter fields on `FilmstripSettings` (`SegmentCount`,
  `FillDirection`, `ContinuousFill`, `SegmentGap`, `OnColorArgb`/`OffColorArgb` â€” all
...
## [0.4.0] â€” 2026-06-03 â€” Batch processing

### Added
- **Phase 5 â€” Batch processing (Batch tab).** `BatchProcessor` (SkiaSharp, no
  ...
  tab â€” **Create** | **Import** | **Batch**). Per-knob frame sizing ("Square knob
...
## [0.3.0] â€” 2026-06-03 â€” Importer + manifest

### Added
- **Phase 2 â€” Filmstrip importer (Import tab).** `FilmstripImporter` (SkiaSharp,
...
- **Phase 3 â€” Manifest export.** A "Also write a skin.json manifest" toggle + a
...
## [0.2.0] â€” 2026-06-03 â€” Rename, Phase 0/1, test project

### Changed
- **Renamed the app FilmstripForge â†’ StripKit** across all docs and code (brand,
  namespaces, `RootNamespace`/`AssemblyName`, `app.manifest`, `avares://` URIs,
  solution/project, folder â†’ `src/StripKit/`). Kept "filmstrip" as the domain noun;
...
### Added
- **Phase 0 verified** â€” `dotnet build` 0/0; GUI boots; load â†’ render â†’ export
  round-trip confirmed (valid 80Ã—5120 strip, clean 270Â° sweep).
- **Phase 1 â€” drag-and-drop.** The Create preview is a drop zone; the button and the
  drop share `MainWindowViewModel.LoadSourceFromPath` (no duplicated load logic).
- **Phase 4 (brought forward) â€” test project** `tests/StripKit.Tests`: renderer
  golden-image baselines, VM load-path tests, headless drop-zone test.

## [0.1.0] â€” 2026-06-02 â€” Scaffold handoff
